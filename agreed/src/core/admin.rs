    ...
    NonVoterState<'a, D, R, N, S>
{
    ...
    pub(super) async fn handle_init_with_config(
        ...
        self.core.membership = MembershipConfig { members };
    ...
    LeaderState<'a, D, R, N, S>
{
    ...
    #[tracing::instrument(level = "trace", skip(self))]
    pub(super) async fn handle_config_change_committed(
        &mut self,
        index: u64,
    ) -> Result<(), RaftError> {
        let node_to_remove = match self.consensus_state {
            ConsensusState::ConfigChange { node, operation }
                if operation == ConfigChangeOperation::RemovingNode =>
            {
                Some(node)
            }
            _ => None,
    ...
    async fn propagate_membership_change(&mut self, tx: ChangeMembershipTx) {
        ...
        self.config_change_committed_cb
            .push(rx_config_change_committed);
